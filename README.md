# Signal Protocol E2EE System with Agentic AI Test Suite

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.0-blue.svg)](https://www.typescriptlang.org/)
[![Deno](https://img.shields.io/badge/Deno-1.46-black.svg)](https://deno.land/)
[![Docker](https://img.shields.io/badge/Docker-Ready-blue.svg)](https://www.docker.com/)

An implementation of the **Signal Protocol** with an **Agentic AI Test Suite** for system maintenance and diagnostics. This project demonstrates end-to-end encrypted messaging between AI agents and human operators, featuring comprehensive automated testing, real-time monitoring, and AI-powered diagnostics.

## 🌟 Key Features

### Core Capabilities
- **🔐 End-to-End Encryption (E2EE)** - Signal Protocol implementation with Double Ratchet Algorithm
- **🤖 AI-Powered Diagnostics** - GPT-4o-mini powered system maintenance agent
- **📊 Agentic Test Suite** - 15 comprehensive tests across Signal, E2EE, AI, and Web API categories
- **💬 Real-time Secure Chat** - E2EE messaging between Alice (user) and SysMaint AI
- **📈 Live Monitoring** - Real-time test execution with progress tracking
- **📋 Test History & Analytics** - SQLite-backed test result storage with export capabilities
- **🔍 Anomaly Detection** - AI-driven system health monitoring and issue detection

### Security Features
- **X3DH Key Agreement** - Extended Triple Diffie-Hellman for session establishment
- **Double Ratchet Algorithm** - Forward secrecy and future secrecy for message encryption
- **Curve25519 & Kyber** - Post-quantum cryptography with hybrid key exchange
- **AES-256-CBC + HMAC-SHA256** - Authenticated encryption with integrity verification
- **Signal Protocol Library** - Official `@signalapp/libsignal-client` implementation

## 🏗️ System Architecture

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           HIGH-LEVEL ARCHITECTURE                            │
└─────────────────────────────────────────────────────────────────────────────┘

                                  ┌─────────────┐
                                  │   CLIENT    │
                                  │   (Alice)   │
                                  └──────┬──────┘
                                         │
                    ┌────────────────────┼────────────────────┐
                    │                    │                    │
                    ▼                    ▼                    ▼
          ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
          │   Web Browser   │  │   Test Suite    │  │     Chat UI     │
          │   (Next.js)     │  │   Dashboard     │  │   (E2EE)        │
          │                 │  │                 │  │                 │
          │ • 15 Agentic    │  │ • Real-time     │  │ • Signal Proto  │
          │   Tests         │  │   Progress      │  │ • Send/Recv     │
          │ • Test History  │  │ • Export JSON   │  │ • AI Responses  │
          │ • AI Overview   │  │ • Notifications │  │ • E2EE Indic.   │
          └────────┬────────┘  └────────┬────────┘  └────────┬────────┘
                   │                    │                    │
                   └────────────────────┼────────────────────┘
                                        │
                                        ▼
                         ┌──────────────────────────┐
                         │     HTTP API Layer       │
                         │    (Next.js Routes)      │
                         │                          │
                         │  /api/chat        /api/e2ee/send   │
                         │  /api/e2ee/pull   /api/test/*      │
                         └───────────┬──────────────┘
                                     │
           ┌─────────────────────────┼─────────────────────────┐
           │                         │                         │
           ▼                         ▼                         ▼
┌─────────────────────┐  ┌─────────────────────┐  ┌─────────────────────┐
│   SysMaint Agent    │  │   Signal Protocol   │  │   State Database    │
│   (AI Processor)    │  │   (E2EE Engine)     │  │   (SQLite)          │
│                     │  │                     │  │                     │
│ • GPT-4o-mini       │  │ • X3DH Handshake    │  │ • Test Results      │
│ • Tool Selection    │  │ • Double Ratchet    │  │ • Message History   │
│ • Diagnostics       │  │ • AES-256-CBC       │  │ • Session State     │
│ • Report Gen        │  │ • HMAC-SHA256       │  │ • Metrics           │
└──────────┬──────────┘  └──────────┬──────────┘  └──────────┬──────────┘
           │                        │                        │
           │                        ▼                        │
           │           ┌─────────────────────┐               │
           │           │   Relay Server      │               │
           │           │   (WebSocket)       │               │
           │           │                     │               │
           │           │ • Message Routing   │               │
           │           │ • E2EE Envelopes    │               │
           │           │ • Presence Mgmt     │               │
           │           └──────────┬──────────┘               │
           │                      │                          │
           ▼                      ▼                          ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                         Deno Microservice                               │
│                         (Diagnostic Reports)                            │
│                                                                         │
│  • Scheduled Analysis (Deno.cron)                                       │
│  • LLM-Powered Report Generation                                        │
│  • Test Result Analysis                                                 │
│  • AI Recommendations                                                   │
└─────────────────────────────────────────────────────────────────────────┘
```

## 🔐 Signal Protocol Implementation

### Protocol Stack

```
┌─────────────────────────────────────────────────────────────────┐
│                    SIGNAL PROTOCOL STACK                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │              APPLICATION LAYER                           │    │
│  │  • Messages, Attachments, Group Chats                   │    │
│  └─────────────────────────────────────────────────────────┘    │
│                              │                                   │
│  ┌───────────────────────────▼───────────────────────────┐      │
│  │           DOUBLE RATCHET ALGORITHM                     │      │
│  │  ┌─────────────┐    ┌─────────────┐    ┌──────────┐  │      │
│  │  │ Root Key    │───▶│ Chain Keys  │───▶│ Message  │  │      │
│  │  │ Derivation  │    │ (Sending)   │    │ Keys     │  │      │
│  │  └─────────────┘    └─────────────┘    └──────────┘  │      │
│  │                              │                        │      │
│  │  ┌───────────────────────────▼────────────────────┐   │      │
│  │  │           DIFFIE-HELLMAN RATCHET                │   │      │
│  │  │  • New ephemeral keys per message               │   │      │
│  │  │  • Forward secrecy guarantee                    │   │      │
│  │  └────────────────────────────────────────────────┘   │      │
│  └─────────────────────────────────────────────────────────┘      │
│                              │                                   │
│  ┌───────────────────────────▼───────────────────────────┐      │
│  │         X3DH KEY AGREEMENT (Initial Setup)             │      │
│  │                                                          │      │
│  │   Alice                          Bob                     │      │
│  │     │                              │                     │      │
│  │     │  ┌──────────────────────┐    │                     │      │
│  │     ├──│ Identity Key (IK_A)  │────┤                     │      │
│  │     │  └──────────────────────┘    │                     │      │
│  │     │  ┌──────────────────────┐    │                     │      │
│  │     ├──│ Signed PreKey (SPK)  │────┤                     │      │
│  │     │  └──────────────────────┘    │                     │      │
│  │     │  ┌──────────────────────┐    │                     │      │
│  │     ├──│ One-Time PreKey (OPK)│────┤                     │      │
│  │     │  └──────────────────────┘    │                     │      │
│  │     │  ┌──────────────────────┐    │                     │      │
│  │     └──│ Kyber PreKey (PQ)    │────┘                     │      │
│  │        └──────────────────────┘                          │      │
│  │                                                          │      │
│  │  DH1 = DH(IK_A, SPK_B)        DH2 = DH(EK_A, IK_B)      │      │
│  │  DH3 = DH(EK_A, SPK_B)        DH4 = DH(EK_A, OPK_B)     │      │
│  │                                                          │      │
│  │  SK = KDF(DH1 || DH2 || DH3 || DH4 || PQK)              │      │
│  └─────────────────────────────────────────────────────────┘      │
│                              │                                   │
│  ┌───────────────────────────▼───────────────────────────┐      │
│  │           CRYPTOGRAPHIC PRIMITIVES                     │      │
│  │                                                          │      │
│  │  • Curve25519 (Elliptic Curve)  - Key Exchange          │      │
│  │  • Kyber-768 (Post-Quantum)     - Quantum Resistance    │      │
│  │  • AES-256-CBC                  - Symmetric Encryption  │      │
│  │  • HMAC-SHA256                  - Message Integrity     │      │
│  │  • SHA-256                      - Hashing               │      │
│  └─────────────────────────────────────────────────────────┘      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### E2EE Message Flow

```
Alice (Web UI)                                    SysMaint AI Agent
      │                                                   │
      │  1. COMPOSE MESSAGE                               │
      │     "Check system status"                         │
      │                                                   │
      │  2. ENCRYPT (Signal Protocol)                     │
      │     ┌─────────────────────────────┐              │
      │     │ • Get session with Bob      │              │
      │     │ • Advance sending chain     │              │
      │     │ • Derive message key (KDF)  │              │
      │     │ • AES-256-CBC encryption    │              │
      │     │ • HMAC-SHA256 authentication│              │
      │     │ • Base64 encode             │              │
      │     └─────────────────────────────┘              │
      │                                                   │
      │  3. SEND TO RELAY                                 │
      │     POST /api/e2ee/send                           │
      │     { encryptedMessage, to: "sysmaint" }          │
      │──────────────────────────────────────────────────▶│
      │                                                   │
      │                        4. ROUTE TO RECIPIENT      │
      │                        (WebSocket/HTTP)           │
      │                                                   │
      │  5. NOTIFY ALICE                    6. PULL MESSAGES
      │     "Message sent ✓"               (Polling/WebSocket)
      │                                                   │
      │                        7. DECRYPT                 │
      │                        ┌──────────────────────┐   │
      │                        │ • Verify HMAC        │   │
      │                        │ • AES-256-CBC decrypt│   │
      │                        │ • Advance recv chain │   │
      │                        │ • Plaintext: "Check..│   │
      │                        └──────────────────────┘   │
      │                                                   │
      │                        8. PROCESS WITH GPT-4o-mini│
      │                        "System status: All OK"    │
      │                                                   │
      │  10. RECEIVE ENCRYPTED ◀──────────────────────────│
      │      { encryptedReply, from: "sysmaint" }         │
      │                                                   │
      │  11. DECRYPT & DISPLAY                            │
      │      🔐 E2EE Message: "System status: All OK"     │
      ▼                                                   ▼
```

## 🤖 Agentic AI Test Suite

### Test Categories

```
┌─────────────────────────────────────────────────────────────────────────┐
│                     AGENTIC AI TEST SUITE ARCHITECTURE                   │
└─────────────────────────────────────────────────────────────────────────┘

    ┌──────────────────────────────────────────────────────────┐
    │                   TEST ORCHESTRATOR                       │
    │  ┌────────────────────────────────────────────────────┐  │
    │  │  • Sequential/Parallel Execution                   │  │
    │  │  • Retry Logic (up to 3 attempts)                  │  │
    │  │  • Progress Tracking via SSE                       │  │
    │  │  • Cost Calculation & Budget Monitoring            │  │
    │  │  • Result Persistence (SQLite)                     │  │
    │  └────────────────────────────────────────────────────┘  │
    └────────────────────┬─────────────────────────────────────┘
                         │
    ┌────────────────────┼────────────────────┐
    │                    │                    │
    ▼                    ▼                    ▼
┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐
│  SIGNAL  │    │   E2EE   │    │    AI    │    │   WEB    │
│ PROTOCOL │    │  HEALTH  │    │  AGENT   │    │   API    │
├──────────┤    ├──────────┤    ├──────────┤    ├──────────┤
│1. ID Gen │    │5. Encrypt│    │9. Tool   │    │13. DB    │
│2. Prekeys│    │6. PreKey │    │  Select  │    │   Query  │
│3. Session│    │  Format  │    │10. History│   │14. Chat  │
│4. Persist│    │7. Ratchet│    │11. Anomaly│   │   API    │
│          │    │8. Tamper │    │12. Context│   │          │
│          │    │  Detect  │    │13. Multi │    │          │
│          │    │          │    │  Tool    │    │          │
└──────────┘    └──────────┘    └──────────┘    └──────────┘
```

### Test Details

#### Signal Protocol Tests (Tests 1-4)

| Test | Name | Description | Duration | Crypto |
|------|------|-------------|----------|--------|
| 1 | Signal Identity Generation | Generate Curve25519 identity key pair and registration ID | 300ms | Curve25519 |
| 2 | Prekey Bundle Generation | Generate signed prekeys, one-time prekeys, and post-quantum Kyber prekeys | 400ms | Ed25519, Kyber-768 |
| 3 | X3DH Session Establishment | Establish Signal session using Extended Triple Diffie-Hellman key agreement | 500ms | X3DH |
| 4 | Session Persistence | Verify sessions survive database close/reopen operations | 200ms | SQLite |

#### E2EE Health Tests (Tests 5-8)

| Test | Name | Description | Duration | Validation |
|------|------|-------------|----------|------------|
| 5 | E2EE Encryption/Decryption | Encrypt using AES-CBC + HMAC-SHA256, verify ciphertext differs from plaintext | 200ms | Roundtrip |
| 6 | PreKey Message Format | Verify first message uses PreKey format with correct fields (type 2 or 3) | 300ms | Format |
| 7 | Double Ratchet Advancement | Verify forward secrecy by sending 5 messages with unique encryption keys each time | 800ms | Forward Secrecy |
| 8 | Integrity Verification | Verify HMAC-SHA256 authentication detects tampered ciphertext and rejects decryption | 100ms | Integrity |

#### AI Agent Tests (Tests 9-13)

| Test | Name | Description | Duration | Cost |
|------|------|-------------|----------|------|
| 9 | AI Tool Selection | Verify AI correctly routes 'check status' queries to the status tool | 1200ms | $0.0003 |
| 10 | Historical Data Query | Verify AI queries historical metrics with correct time range parameters | 1500ms | $0.0004 |
| 11 | Anomaly Detection Query | Verify AI routes 'find problems' queries to the anomaly detection tool | 900ms | $0.0002 |
| 12 | Conversation Context | Verify AI maintains context across consecutive messages in same session | 1100ms | $0.0003 |
| 13 | Multi-Tool Reasoning | Verify AI chains status + history + anomaly tools for complex diagnostic queries | 2300ms | $0.0005 |

#### Web API Tests (Tests 14-15)

| Test | Name | Description | Duration | Component |
|------|------|-------------|----------|-----------|
| 14 | Database Query Operations | Test SQLite state database insert, query, and indexing operations | 100ms | SQLite |
| 15 | Chat API End-to-End | Test chat API with Signal Protocol encryption and OpenAI response | 500ms | Full Stack |

### Test Execution Flow

```
┌──────────────────────────────────────────────────────────────────┐
│                     TEST EXECUTION FLOW                           │
└──────────────────────────────────────────────────────────────────┘

User clicks                                    ┌────────────────────┐
"Run All Tests"                                │  Test Dashboard    │
     │                                         │  (Next.js UI)      │
     ▼                                         └─────────┬──────────┘
┌──────────────────┐                                    │
│  1. API Request  │◀───────────────────────────────────┘
│     POST /api/   │
│     test/run     │
└────────┬─────────┘
         │
         ▼
┌──────────────────────────────────┐
│  2. EventSource Connection       │
│     (Server-Sent Events)         │
│                                  │
│     Client ◀─── data: {...}     │
│     Client ◀─── data: {...}     │
│     Client ◀─── data: {...}     │
└────────┬─────────────────────────┘
         │
         ▼
┌──────────────────────────────────┐
│  3. Test Runner Initialization   │
│                                  │
│     • Create test context        │
│     • Initialize in-memory DB    │
│     • Set up Signal Protocol     │
└────────┬─────────────────────────┘
         │
         ▼
┌──────────────────────────────────────────────────────────────┐
│              SEQUENTIAL TEST EXECUTION                        │
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐      │
│  │ Test 1      │───▶│ Test 2      │───▶│ Test 3      │───▶  │
│  │ Signal ID   │    │ Prekeys     │    │ Session     │      │
│  └─────────────┘    └─────────────┘    └─────────────┘      │
│        │                  │                  │               │
│        ▼                  ▼                  ▼               │
│   ┌──────────┐       ┌──────────┐       ┌──────────┐        │
│   │ Attempt 1│       │ Attempt 1│       │ Attempt 1│        │
│   │ (Success)│       │ (Success)│       │ (Success)│        │
│   └──────────┘       └──────────┘       └──────────┘        │
│                                                               │
│   ┌───────────────────────────────────────────────────────┐  │
│   │ RETRY LOGIC (if needed)                               │  │
│   │                                                       │  │
│   │  Attempt 1 ──▶ Failed? ──▶ Wait 1s ──▶ Attempt 2     │  │
│   │     Failed? ──▶ Wait 2s ──▶ Attempt 3 (Final)        │  │
│   └───────────────────────────────────────────────────────┘  │
│                                                               │
└──────────────────────────────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────┐
│  4. Result Aggregation           │
│                                  │
│     • Calculate pass/fail rates  │
│     • Sum total costs            │
│     • Aggregate duration         │
│     • Store in SQLite            │
└────────┬─────────────────────────┘
         │
         ▼
┌──────────────────────────────────┐
│  5. Browser Notification         │
│                                  │
│     ✓ Test run completed!        │
│     15 passed, 0 failed          │
│     Total cost: $0.00025         │
└──────────────────────────────────┘
```

## 💬 Alice ↔ SysMaint AI Chat Architecture

### Chat Message Flow

```
┌─────────────────────────────────────────────────────────────────────────┐
│                     SECURE CHAT MESSAGE FLOW                             │
└─────────────────────────────────────────────────────────────────────────┘

┌──────────────────┐                                ┌──────────────────┐
│     ALICE        │                                │   SYSMAINT AI    │
│   (Web Browser)  │                                │   (Agent)        │
└────────┬─────────┘                                └────────┬─────────┘
         │                                                  │
         │  STEP 1: COMPOSE & ENCRYPT                       │
         │  ┌─────────────────────────────────────┐         │
         │  │ User types: "Check system status"   │         │
         │  │                                     │         │
         │  │ Signal Protocol Operations:         │         │
         │  │ • Get session for "sysmaint"        │         │
         │  │ • Advance sending chain key         │         │
         │  │ • Derive message key (KDF)          │         │
         │  │ • AES-256-CBC encrypt               │         │
         │  │ • HMAC-SHA256 authenticate          │         │
         │  │ • Increment message counter         │         │
         │  │                                     │         │
         │  │ Output: EncryptedMessage {          │         │
         │  │   type: 3 (SignalMessage),          │         │
         │  │   body: "base64(ciphertext)",       │         │
         │  │   counter: N                        │         │
         │  │ }                                   │         │
         │  └─────────────────────────────────────┘         │
         │                                                  │
         │  POST /api/e2ee/send                             │
         │  { to: "sysmaint", message: encryptedMsg }       │
         │─────────────────────────────────────────────────▶│
         │                                                  │
         │                        STEP 2: RECEIVE & DECRYPT │
         │                        ┌───────────────────────┐ │
         │                        │ • Verify HMAC         │ │
         │                        │ • AES-256-CBC decrypt │ │
         │                        │ • Advance recv chain  │ │
         │                        │ • Plaintext: "Check.. │ │
         │                        └───────────────────────┘ │
         │                                                  │
         │                        STEP 3: AI PROCESSING     │
         │                        ┌───────────────────────┐ │
         │                        │ GPT-4o-mini analysis  │ │
         │                        │ • Tool selection      │ │
         │                        │ • Status query        │ │
         │                        │ • Response generation │ │
         │                        │                       │ │
         │                        │ "System is healthy:   │ │
         │                        │  CPU: 15%, Memory: 60%│ │
         │                        └───────────────────────┘ │
         │                                                  │
         │                        STEP 4: ENCRYPT RESPONSE  │
         │                        ┌───────────────────────┐ │
         │                        │ Same E2EE process     │ │
         │                        │ • Encrypt response    │ │
         │                        │ • Update ratchet      │ │
         │                        └───────────────────────┘ │
         │                                                  │
         │  Encrypted reply stored in database              │
         │  (Alice will poll/pull to receive)               │
         │◀─────────────────────────────────────────────────│
         │                                                  │
         │  STEP 5: ALICE RECEIVES                          │
         │  GET /api/e2ee/pull                              │
         │─────────────────────────────────────────────────▶│
         │                                                  │
         │  { messages: [encryptedReply] }                  │
         │◀─────────────────────────────────────────────────│
         │                                                  │
         │  STEP 6: DECRYPT & DISPLAY                       │
         │  ┌─────────────────────────────────────┐         │
         │  │ • Decrypt with Signal Protocol      │         │
         │  │ • Verify authenticity               │         │
         │  │ • Display with 🔐 E2EE indicator    │         │
         │  │                                     │         │
         │  │ 🤖 SysMaint: "System is healthy...  │         │
         │  │ 🔐 E2EE verified                    │         │
         │  └─────────────────────────────────────┘         │
         │                                                  │
```

### UI Components

```
┌─────────────────────────────────────────────────────────────────────────┐
│                     CHAT WINDOW UI LAYOUT                                │
└─────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────┐
│ 💬 Alice ↔ SysMaint AI                              ● Encrypted      │  ← Header
│ 🔐 E2EE Signal Protocol                                              │
├──────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  🤖                                                    👩              │
│  ┌──────────────────────────────────────┐                            │
│  │ 🔐 Welcome! I'm SysMaint AI.         │                            │
│  │ All messages are E2EE encrypted.     │                            │
│  │                                      │                            │
│  │ Click any test card to learn more!   │                            │
│  └──────────────────────────────────────┘                            │
│                                    ┌──────────────────────────────┐  │
│                                    │ What's test #5 about?       │  │
│                                    │ 🔐                           │  │
│                                    └──────────────────────────────┘  │
│                                                                      │
│  🤖                                                                  │
│  ┌──────────────────────────────────────────────────────────────┐    │
│  │ 🔐 📖 E2EE Encryption/Decryption                             │    │
│  │                                                             │    │
│  │ Encrypt message using AES-CBC + HMAC-SHA256 and verify     │    │
│  │ ciphertext differs from plaintext.                         │    │
│  │                                                             │    │
│  │ ⏱️ ~0.2s  💰 ~$0.0000                                      │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                      │  ← Messages Area
│  🤖                                                                  │     (Scrollable)
│  ┌────────────────────────┐                                          │
│  │ ●●● (typing)          │                                          │
│  └────────────────────────┘                                          │
│                                                                      │
├──────────────────────────────────────────────────────────────────────┤
│ ┌──────────────────────────────────────────────────────┬───────────┐ │
│ │ Ask about any test...                               │     ➤     │ │  ← Input Area
│ └──────────────────────────────────────────────────────┴───────────┘ │
└──────────────────────────────────────────────────────────────────────┘

Dimensions:
- Desktop: 800px height, 640px messages area
- Mobile: 700px height, 540px messages area
```

## 🚀 Quick Start

### Prerequisites

- Docker and Docker Compose
- Node.js 20+ (for local development)
- Deno 1.46+ (for diagnostic microservice)
- OpenAI API key

### Installation

```bash
# Clone the repository
git clone <repository-url>
cd SignalProtocolSysMaintAgentv04Test01

# Set up environment variables
cp .env.example .env
# Edit .env and add your OPENAI_API_KEY

# Start all services
docker compose up -d

# Wait for services to initialize
sleep 10
```

### Accessing the Application

- **Web Dashboard**: http://localhost:3000
- **Test Suite**: http://localhost:3000/test
- **Relay Server**: http://localhost:8080
- **SysMaint Agent**: http://localhost:3001

### Running Tests

```bash
# Run all tests via API
curl -s http://localhost:3000/api/test/run

# Check test results
curl -s http://localhost:3000/api/test/history

# Export test results
curl -s "http://localhost:3000/api/test/export?runId=<runId>&format=structured"
```

## 📊 System Monitoring & Diagnostics

### Deno Diagnostic Microservice

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    DENO DIAGNOSTIC MICROSERVICE                          │
└─────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────┐
│  Deno Runtime                                                        │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  HTTP Server (Port 8000)                                       │  │
│  │  ┌──────────────────────────────────────────────────────────┐  │  │
│  │  │  Endpoints:                                              │  │  │
│  │  │  • GET  /health          - Health check                  │  │  │
│  │  │  • POST /analyze         - Analyze test runs             │  │  │
│  │  │  • POST /report          - Generate diagnostic report    │  │  │
│  │  └──────────────────────────────────────────────────────────┘  │  │
│  │                                                                    │  │
│  │  ┌──────────────────────────────────────────────────────────┐  │  │
│  │  │  Deno.cron Scheduler                                     │  │  │
│  │  │  • Daily at 00:00 - Generate overnight report            │  │  │
│  │  │  • Hourly - Check system health                          │  │  │
│  │  │  • Weekly - Full system diagnostic                       │  │  │
│  │  └──────────────────────────────────────────────────────────┘  │  │
│  └────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────┘
```

### AI-Powered Analysis

The diagnostic microservice uses GPT-4o-mini to:

1. **Analyze Test Patterns**: Identify failing tests, trends, and correlations
2. **Generate Reports**: Natural language summaries of system health
3. **Recommend Actions**: AI-suggested fixes for detected issues
4. **Predict Issues**: Early warning system based on metric patterns

## 🛠️ Development

### Project Structure

```
SignalProtocolSysMaintAgentv04Test01/
├── apps/
│   ├── sysmaint-web/          # Next.js web application
│   │   ├── app/               # App router pages
│   │   ├── lib/               # Utility libraries
│   │   └── components/        # React components
│   ├── sysmaint-agent/        # AI agent service
│   ├── cli/                   # Command-line interface
│   └── diag-probe/            # Diagnostic probe
├── packages/
│   ├── signal-core/           # Signal Protocol implementation
│   ├── sysmaint-protocol/     # Protocol definitions
│   ├── shared/                # Shared types and utilities
│   └── relay-server/          # Message relay server
├── infra/
│   ├── deno/                  # Deno microservices
│   └── diagnostics/           # Diagnostic tools
└── docker-compose.yml         # Orchestration
```

### Adding New Tests

1. Define test in `apps/sysmaint-web/lib/test-suite/metadata.ts`
2. Implement test in `apps/sysmaint-web/lib/test-suite/signal-e2ee.ts` (or appropriate file)
3. Update test runner to include the new test
4. Rebuild and verify

### Environment Variables

```bash
# Required
OPENAI_API_KEY=sk-...

# Optional (with defaults)
RELAY_SERVER_URL=http://localhost:8080
SYSMAINT_AGENT_URL=http://localhost:3001
DATABASE_PATH=./data/state.db
DENO_SERVICE_URL=http://localhost:8000
```

## 🔒 Security Considerations

### Key Management

- **Identity Keys**: Stored in SQLite with passphrase protection
- **Session Keys**: Ephemeral, rotated per-message via Double Ratchet
- **PreKeys**: One-time use, automatically replenished

### Threat Model

| Threat | Mitigation |
|--------|------------|
| Eavesdropping | E2EE with Signal Protocol |
| Man-in-the-Middle | X3DH with identity key verification |
| Quantum Computing | Kyber post-quantum key exchange |
| Replay Attacks | Message counters and timestamps |
| Forward Secrecy | Double Ratchet with ephemeral keys |

## 📈 Performance Metrics

### Test Suite Performance

- **Total Test Time**: ~10 seconds (all 15 tests)
- **Average Test Time**: 600ms
- **Success Rate**: 100% (with retry logic)
- **AI Cost per Run**: ~$0.00025

### Signal Protocol Performance

- **Key Generation**: ~50ms
- **Session Establishment**: ~100ms
- **Encryption**: ~5ms per message
- **Decryption**: ~5ms per message
- **Database Operations**: ~10ms

## 📄 License

This project is licensed under the MIT License - see [LICENSE](LICENSE) file for details.

## Acknowledgments

- **Signal Foundation**: For the Signal Protocol and libsignal-client
- **OpenAI**: For GPT-4o-mini API
- **Deno**: For the secure TypeScript runtime
- **Next.js Team**: For the React framework

**Built with 🔐 encryption and 🤖 AI for secure system maintenance.**
